-- Projections (standard odds) where the player exceeded the line in each of last 5 games.
-- Requires: prizepicks_projection, prizepicks_player, player, player_stat.
-- Link: prizepicks_player.nba_player_id -> player.player_id (or by name: display_name = display_first_last).

USE [Props]
GO

WITH
-- Last 5 games per NBA player with all stat columns
last5 AS (
    SELECT
        player_id,
        game_date,
...
-- Standard single-player projections + NBA player_id (direct link or name fallback)
proj AS (
    SELECT
        p.projection_id,
        p.line_score,
...
-- Stat value for each (projection, game) from the column that matches stat_type_name
game_values AS (
    SELECT
        proj.projection_id,
        proj.display_name,
...
SELECT
    ...
    END AS risk
FROM qualified q
ORDER BY q.stat_type_name, q.display_name, q.line_score;